...
Finally, Section~\ref{sec:selfcheck}'s clause leaves the two deployments incomparable on
coherence, and now says so rather than obscuring it. Deployment B is evaluated
over a real dependency graph at 0.653 edge coverage. Deployment A has no graph to
evaluate, and recovering one means migrating it to a format with a dependency
field and declaring structure that currently exists only implicitly in its
chapters. That migration is unperformed, and until it is done this paper reports
one deployment's coherence rather than two.
