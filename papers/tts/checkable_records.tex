...
named them. We also report a defect in our own work: the validator certifies the
higher conformance level for a deployment whose dependency graph its parser
cannot populate, so a coherence check passes over an empty graph. The same
programme requires judgment procedures to prove they are not vacuous, and the
record specification carries no equivalent condition. We state the fix and do not
apply it, because changing conformance semantics for a deployed ledger is the
specification owner's decision rather than the checker's.
\end{abstract}
...
At L3 it conforms. P2 holds over the full range 001 to 091 with zero gaps and
...
The most serious finding in this paper is a defect in our own work, and the
...
We have not implemented it here. Applying it would move a deployed record from
conforming to non-conforming, and that is a decision for the specification's
owner rather than for the checker's author, on the same grounds that a separate
finding elsewhere in this programme is left unrepaired. Reporting the defect and
leaving it open is the behavior the specification asks for. It is also the reason
we report conformance levels for both deployments with the qualification that
Deployment A's L3 is, on the current definition, weaker than it sounds.
...
Finally, Section~\ref{sec:selfcheck}'s defect means the two deployments are not
directly comparable. Deployment B's conformance is evaluated over a real
dependency graph and Deployment A's over an empty one.
