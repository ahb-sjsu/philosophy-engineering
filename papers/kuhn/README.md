...
**§6 now identifies the inquiry arm positively.** "Infrastructure" says only what
it is not. The positive identification is Hart's: the claim ledger is a set of
**secondary rules** for a research programme. Hart's three defects of a regime
with primary rules only map onto three of the four pathologies — uncertainty to
classes and P3, static character to the lifecycle and `withdrawn`/`refuted`
propagation, inefficiency to blast radius by traversal. The payoff is that this
*predicts* the brownfield finding rather than restating it: the Geometric Ethics
corpus had a complete set of primary rules (status tags, retraction culture,
provenance) and no rule saying what a correction **does**, and Hart's account
says such an order can recognise a violation and cannot determine its
consequences. The orphan correction is the structural signature of primary rules
without secondary ones. The file drawer (P2) falls outside the mapping, which is
stated rather than smoothed — jurisprudence has no problem corresponding to cases
never filed.

The historical precedent given is **double-entry bookkeeping**, not Copernicus:
not a theory of commerce, and it made solvency checkable by structural
redundancy. Two cautions are in the text. Sombart's enabling thesis is contested
and the accounting-history literature is actively revising claims of that shape,
so the parallel carries the mechanism and no weight about consequences. And
double entry spread under *external* accountability pressure (Florentine bankers
answerable to their guild), whereas this method is operated by the only party it
constrains — a difference §10 now has to absorb.
...
existing corpus today. P-V follows from §6's accounting precedent and separates
the institutional from the epistemic reading of the mechanism: contributor count
rising with a flat retraction rate confirms it, a falling error rate at constant
scale refutes it.
...
| **Hart (1961)**, primary/secondary rules | The distinction itself; routine application to international law, constitutional orders, informal normative systems | **Searched for an application to research-programme governance and found none.** §6 states the correspondence as "not aware of prior art", not as established-new. |
| Accounting history (Sangster 2025 and the Lakatos-applied-to-bookkeeping literature) | The analogy in the *reverse* direction: MSRP used to explain why double entry displaced single entry | Borrowing the vehicle back is not an argument. §6 uses it for mechanism only. |
...
located for that specific move. The Hart correspondence in §6 is claimed more
weakly still — as unlocated rather than absent — and a reader who knows the
application should treat it as a correction to make.
...
Outputs: `kuhn_paradigm.pdf` (14 pp), `kuhn_paradigm.docx`.
...
- [x] Banned-word grep clean (two hits over both drafts, `genuinely` and `honesty`, removed)
