...
is a change to the medium in which paradigms are recorded, and the paper states
four predictions by which it can be shown wrong.
\end{abstract}
...
programmes, not a revolution, and the paper states four ways it could be shown
wrong.
...
The literature on psychology's credibility revolution has reached the same
verdict about that episode: it is substantial reform of standards of evidence
rather than a change of theory, and the sources that call it a revolution are
using the word in a different sense from Kuhn's \citep{vazire2018}.
...
The inquiry arm lends scope across all of science, which a subfield theory
lacks. A reader who takes both at once perceives something that looks like a
general scientific revolution. Assessed separately, one is an early theory in a
single field and the other is an infrastructure proposal. Neither is a paradigm.
...
wrong. Four predictions follow from the account in \S7, stated so that a
...
today from any conforming ledger. P-II and P-III require a second programme, and
until a programme other than this one adopts the method, none of the four has
been tested.
