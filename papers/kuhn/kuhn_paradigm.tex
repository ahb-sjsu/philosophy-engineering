...
against itself here: no existing specification gives dependency edges the
priority property it gives claims, so the answer describes what a ledger makes
possible and not what any deployment yet shows. That is not a paradigm. It
...
arm has already solved the same problem for transformation families, by
...
result \citep{pedsc}; the inquiry arm has no matching clause for edges. Until it
does, the answer to Feyerabend set out above describes a discipline the
specification does not yet enforce, and should be read as what a claim ledger
makes possible rather than as what any existing one demonstrates.
