...
requires dependencies to be declared before the test would turn that partition
into a property of the record rather than a reconstruction, so the question of
what a refutation strikes acquires a present-tense answer. The paper reports
against itself here. Dependency edges acquired that priority property only after
this argument was written, and in the two existing deployments the fraction of
edges whose declaration can be shown to predate its result is 0.25 and 0.00. The
answer therefore holds in the form a ledger makes available, and no deployment
has yet earned it. That is not a paradigm. It
...
Four limits on this claim must be stated with it, and the first is severe
enough that it bounds the rest.

First, the specification does not require what the argument depends on. Priority
is a property the specification gives to *claims*: a registration must be an
ancestor of its result, and a claim that cannot show this is capped. It gives no
such property to *edges*. Nothing requires a dependency to have been declared
before the test it bears on, nothing records when an edge was declared, and no
check distinguishes a graph wired in advance from one wired afterwards. The
argument above needs the partition to be prospective, and a ledger conforming at
the highest level defined today can have been wired entirely in hindsight.

This is not hypothetical. The companion paper's Deployment A was converted from a
format with no dependency field into one with 27 declared edges, and every edge
was written after the results it organises were known. The checker accepted all
of them. A ledger in that state answers Feyerabend no better than the prose it
replaced, because the core and belt were still separated by someone who already
knew which survived. What the record buys over a reconstruction is the ability
to *show* that the partition preceded the test, and that ability is currently
available for claims and unavailable for the edges between them. The discovery
arm had already solved the same problem for transformation families, by
requiring a late row to carry the date it was written, name the sealed artifact
holding the declaration, and demonstrate that the declaration preceded the
result \citep{pedsc}. The inquiry arm now carries the matching clause for edges
\citep{pecls}: an edge may declare when it was fixed and where, it is classified
by whether that can be shown, and blast radius is computed over the prospective
subgraph separately from the full one. The argument above is therefore checkable
rather than merely asserted, and what it needs is now a measurement instead of a
promise.

The measurement is not flattering, which is the point of taking it. Across the
two deployments the fraction of dependencies whose declaration can be shown to
predate the result they organise is 0.25 and 0.00. Refuting the most-depended-on
claim in the first strikes two claims on the programme's present beliefs and one
on what the record fixed in advance. So the answer to Feyerabend holds in the
form a ledger makes available, and neither existing deployment has yet earned
it. A programme quoting the larger number while claiming the smaller one's
warrant would be committing the error this entire apparatus exists to prevent,
and the gap between the two columns is now a quantity rather than a suspicion.

Second, it addresses one input to Lakatos's appraisal rather than the appraisal
...
Third, localization reaches only as far as the declared conjunction. Duhem and
...
Fourth, the mechanism assumes the repository history is intact. Priority proved
...
P-I now has one measurement, reported in the companion paper. Over the 236
claims of the converted corpus, 75 per cent suspend nothing and 85 per cent
suspend two or fewer, while the largest suspends 112, and the claims at the top
of the tail are the definitions of the underlying mathematical objects rather
than the empirical results. The shape is the predicted one. The measurement does
not settle what P-I was written to settle, and the reason is worth recording: a
heavy tail concentrated on a few claims of high in-degree is what a genuine
foundational core looks like, and it is also what pervasive under-declaration
looks like, and this corpus supplies evidence for both readings. The
discriminator stated above is therefore too weak as written, and separating the
two requires a corpus whose dependency declaration is known to be complete. That
is a defect in the prediction rather than in the result.

P-IV is inexpensive and can be run on the existing corpus. P-II, P-III, and P-V
require a second programme, and until a programme other than this one adopts the
method, none of those three has been tested.
...
sealed registrations and were not mirrored into the registry. The specification
then had no way to say so. It admitted a conforming row or a missing one, which
left the repaired registry and the registry fitted to its results looking alike,
and the finding forced a clause distinguishing them: a late row is admissible
only if it carries the date it was written, names the sealed artifact holding the
declaration, and shows that the declaration precedes the result
\citep{pedsc}. Whether the reference registry has since been repaired under that
clause is not something this paper verifies. A paper arguing that mechanical
checks catch what reading misses should report the case where they caught the
author, and should report it again when the catch changed the specification.
...
This paper carries no measurements, by design: its subject is what may be
concluded from the shape of a record, and the argument stands or falls on the
criteria in \S3 rather than on any corpus. A companion manuscript reports the
engineering side, namely a specification of the four properties, a reference
validator, and what running it against two live corpora found, including three
classes of defect that accurate prose had absorbed silently and one
vacuous-compliance failure in the specification itself. The two are separable.
That paper stands without this argument, and this argument was developed without
access to its measurements. They are read together for one reason: the
blast-radius distribution it reports is the first test of P-I above, and \S8
records both the confirmation and the respect in which the prediction was too
weak to do the work asked of it.
